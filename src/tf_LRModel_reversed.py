...
class LRModel(object):
    ...
    def define_model(self):
        with tf.name_scope('input'):
            self.brain_representation = tf.placeholder(shape=[None, self.input_dim], dtype=tf.float32, name='x-input')
            self.word_representation = tf.placeholder(shape=[None, self.output_dim], dtype=tf.float32, name='y-input')

        with tf.name_scope('model'):
            self.W_1 = tf.Variable(tf.random_uniform([self.input_dim, self.output_dim],minval=-0.1,maxval=0.1),
                                   name='W_1')
            self.bias_1 = tf.Variable(tf.zeros([self.output_dim]), name='bias_1')
            ...
            dropped_input = tf.nn.dropout(self.brain_representation, 1.0)
            # This is the same as y = tf.add(tf.mul(m, x_placeholder), b), but looks nicer
            self.h = tf.matmul(dropped_input, tf.nn.dropout(self.W_1,0.7)) + self.bias_1
            self.y = tf.tanh(self.h)

            self.h_test = tf.matmul(self.brain_representation, self.W_1) + self.bias_1
            self.y_test = tf.tanh(self.h_test)

        with tf.name_scope('training'):
            with tf.name_scope('loss'):
                self.beta = 0.001
                self.mse = tf.losses.huber_loss(labels=self.word_representation,predictions=self.y)
                self.pair_loss = tf.losses.mean_pairwise_squared_error(predictions=self.y,labels=self.word_representation,weights=0.5)

                self.loss = self.mse  + self.pair_loss +self.beta * ( tf.nn.l2_loss(self.W_1) + tf.nn.l2_loss(self.bias_1))

                self.mse_test = tf.losses.mean_squared_error(labels=self.word_representation, predictions=self.y_test)
                self.pair_loss_test = tf.losses.mean_pairwise_squared_error(predictions=self.y_test, labels=self.word_representation)
        ...
        for step in range(self.training_steps):
            ...
            while batch_number <= number_of_batches:
                ...
                loss,_ = self.sess.run([self.loss,self.train_op],
                              feed_dict={self.brain_representation: x_train[indexes[start:end]],
                self.word_representation: y_train[indexes[start:end]]})
            ...
            if step % 100 == 0:
                print(step, self.sess.run([self.loss], feed_dict={self.brain_representation: x_train,
                                                                  self.word_representation: y_train}))
    ...
    def test(self, x_test, y_test):
        ...
        y,loss = self.sess.run([self.y,self.loss_test], feed_dict={self.brain_representation: x_test,
                                                                                 ...
                                                                                 self.word_representation: y_test})
