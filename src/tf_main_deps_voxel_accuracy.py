...
if __name__ == '__main__':
    ...
    for test_word_indices in itertools.combinations(range(len(word_set)), 2):
        ...
        if expSetup.mode == "train":
            ...
            lrm.save_model("model_deps_" + word_set[test_word_indices[0]] + "-" + word_set[test_word_indices[1]])
        else:
            if os.path.isfile("model_deps_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]]+".meta"):
                lrm.load_model("model_deps_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]])
            else:
                lrm.load_model("model_deps_"+word_set[test_word_indices[1]]+"-"+word_set[test_word_indices[0]])
