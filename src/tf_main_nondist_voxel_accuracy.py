...
if __name__ == '__main__':
    ...
    words, x_all, y_all = LRModel.prepare_data(fMRI_file=fMRI_data_path+fMRI_data_filename+args.subject+fMRI_data_postfix,
                                               subject=args.subject,type="non-distributional")
    ...
    for test_word_indices in itertools.combinations(range(len(word_set)), 2):
        ...
        if expSetup.mode == "train":
            ...
            lrm.save_model("../models/model_nondist_" + word_set[test_word_indices[0]] + "-" + word_set[test_word_indices[1]])
        else:
            if os.path.isfile("../models/model_nondist_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]]+".meta"):
                lrm.load_model("../models/model_nondist_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]])
            else:
                lrm.load_model("../models/model_nondist_"+word_set[test_word_indices[1]]+"-"+word_set[test_word_indices[0]])
