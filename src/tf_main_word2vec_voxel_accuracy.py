...
if __name__ == '__main__':
    ...
    expSetup = ExpSetup(learning_rate=0.001,batch_size=29,number_of_epochs=10,mode="train")
    ...
    words, x_all, y_all = LRModel.prepare_data(fMRI_file=fMRI_data_path+fMRI_data_filename+args.subject+fMRI_data_postfix,
                                               subject=args.subject,type="word2vec")
    ...
    for test_word_indices in itertools.combinations(range(len(word_set)), 2):
        ...
        if expSetup.mode == "train":
            ...
            lrm.save_model("../models/model_word2vec_" + word_set[test_word_indices[0]] + "-" + word_set[test_word_indices[1]])
        else:
            if os.path.isfile("../models/model_word2vec_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]]+".meta"):
                lrm.load_model("../models/model_word2vec_"+word_set[test_word_indices[0]]+"-"+word_set[test_word_indices[1]])
            else:
                lrm.load_model("../models/model_word2vec_"+word_set[test_word_indices[1]]+"-"+word_set[test_word_indices[0]])
